        ...
        await launchPayment(
          ...
          async (successData) => {
            try {
              ...
              const duration = plan.id === 'lifetime' ? 999 : parseInt(plan.duration);
              const subRes = await fetch("http://localhost:3000/api/v1/prepyatra/subscription", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                  userId: user.userId,
              ...
              if (subData.status) {
                ...
                alert("Payment succeeded, but subscription activation failed: " + subData.message);
              }
            } catch (err) {
          ...
          <div className='grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto'>
            {plans.map((plan) => (
              ...
              >
                ...
                >
                  {loading && selectedPlan === plan.id ? "Redirecting..." : plan.buttonText}
                </button>
              </div>
            ))}
          </div>
