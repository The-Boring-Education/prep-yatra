...
const ChallengeLogsModal = ({isOpen, onClose, challenge}: ChallengeLogsModalProps) => {
  ...
  const generateSocialMessage = (log: ChallengeLog, templateIndex: number = 0) => {
    const progressPercentage = Math.round((log.day / challenge.totalDays) * 100);
...
Spent ${log.hoursSpent} hours grinding, and honestly feeling good about the progress! 
...
Check out Prep Yatra if you want to start your own challenge! ${appUrl}`,
      ...
      `📚 Learning Update: Day ${log.day}/${challenge.totalDays} - ${challenge.name}
...
${challenge.category ? `#${challenge.category} ` : ""}#Motivation #GrowthMindset #PrepYatra #LearningJourney`
  ...
  const shareToSocial = (platform: string) => {
    const message = generateSocialMessage(selectedTemplate);
  ...
  const calculateOverallProgress = () => {
    ...
    return Math.round((totalDays / (challenge.totalDays * logs.length)) * 100);
  };

  const totalHoursSpent = logs.reduce((sum, log) => sum + log.hoursSpent, 0);
  const averageHoursPerDay = logs.length > 0 ? (totalHoursSpent / logs.length).toFixed(1) : 0;
        ...
        {!showSocialPreview ? (
          <>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-semibold text-white">Progress Logs ({logs.length})</h3>
                {logs.length > 0 && (
                  <Button
              ...
              {loading ? (
                ...
                <Card className="bg-gray-800/30 border-gray-600">
                  <CardContent className="pt-6 text-center">
                    ...
                    <p className="text-gray-300 mb-2">No progress logs yet</p>
                    <p className="text-gray-400 text-sm">Start logging your daily progress to see your journey here!</p>
                  </CardContent>
                </Card>
              ) : (
                <div className="space-y-3">
                  {logs.map((log) => (
                    <Card key={log._id} className="bg-gray-800/30 border-gray-600">
                      <CardHeader className="pb-3">
                        <div className="flex items-center justify-between">
                      ...
                      <CardContent className="space-y-3">
                        <div>
                          <h4 className="text-white font-medium mb-2">What I worked on:</h4>
                          <p className="text-gray-300 text-sm">{log.progressText}</p>
                        </div>
            ...
            <DialogFooter>
              <Button onClick={onClose} variant="outline" className="border-gray-300 text-white hover:bg-gray-100 hover:text-gray-900">
                Close
              </Button>
            </DialogFooter>
          </>
