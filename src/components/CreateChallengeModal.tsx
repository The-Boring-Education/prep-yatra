...
import { CHALLENGE_TEMPLATES, ChallengeTemplate, CreateChallengeDTO } from '@/types/challenges';
...
const CreateChallengeModal = ({
  ...
  const handlePredefinedChallengeSelect = async (template: ChallengeTemplate) => {
    setLoading(true);
    try {
      ...
      await challengesService.create(challengeData);
  ...
  const predefinedChallenges = [
    {
      template: CHALLENGE_TEMPLATES[0], // Python challenge
      icon: <Code className="w-8 h-8 text-blue-400" />,
      gradient: 'from-blue-500 to-cyan-500',
      color: 'blue'
    },
    {
      template: CHALLENGE_TEMPLATES[1], // Java challenge
      icon: <Coffee className="w-8 h-8 text-orange-400" />,
      gradient: 'from-orange-500 to-red-500',
      color: 'orange'
    },
    {
      template: CHALLENGE_TEMPLATES[2], // Internship challenge
        ...
        <Tabs value={selectedTab} onValueChange={setSelectedTab} className="w-full">
          ...
          <TabsContent value="predefined" className="space-y-4 mt-6">
            <div className="grid gap-4">
              {predefinedChallenges.map(({ template, icon, gradient, color }) => (
                <Card
                  key={template.id}
                  ...
                  onClick={() => handlePredefinedChallengeSelect(template)}
                >
