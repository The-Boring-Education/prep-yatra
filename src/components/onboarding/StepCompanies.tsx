...
const StepCompanies: React.FC<Props> = ({ formData, toggleArrayField }) => (
  <div className='space-y-6'>
    <div className='text-center mb-8'>
      <h1 className='text-2xl font-bold text-white mb-2'>Which companies interest you? 🏢</h1>
      <p className='text-gray-300'>Select all that apply - we'll customize questions accordingly</p>
    </div>
    <div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
      {COMPANY_TYPES.map((company) => (
        ...
        >
          ...
          <div className='font-medium text-white mb-1'>{company.label}</div>
          <div className='text-sm text-gray-300'>{company.description}</div>
        </button>
      ))}
    </div>
